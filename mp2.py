    ...
    def execute(self, searchMethod, granularity, trajectory, saveImage, saveMaze):        
        ...
        while self.running:
            ...
            if self.__human:     
                ...
                if (keys[K_z]):
                    print("z: ", granularity if isValueInBetween(self.armLimits[ALPHA], alpha+granularity) else 0)
                    alpha += granularity if isValueInBetween(self.armLimits[ALPHA], alpha+granularity) else 0

                if (keys[K_x]): 
                    print("x: ", granularity if isValueInBetween(self.armLimits[ALPHA], alpha-granularity) else 0)
                    alpha -= granularity if isValueInBetween(self.armLimits[ALPHA], alpha-granularity) else 0
